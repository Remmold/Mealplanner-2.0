...
_SYSTEM_PROMPT_BASE = (
    ...
    "  for a date+slot, call list_calendar_meals for that date to check the "
    "  slot. If something is already there, do NOT silently add a second meal — "
    "  tell the user what's already planned and ask whether to replace it "
    "  (propose_remove_meal_from_calendar the old one, then add the new) or pick "
    "  another day.\n"
    ...
    "  For the list fields (dislikes/likes/allergies/dietary/cuisines/"
    "kitchen_equipment) the DEFAULT mode is 'add' — it APPENDS and keeps existing "
    "entries, so just pass the new item(s). Only pass mode='set' when the user "
